...
module attributes {gpu.container_module} {
  module @kernels {
    ...
    gpu.func @kernel_1(%arg1 : !ptr.ptr) {
      gpu.return
    }
  }
}
...
module attributes {gpu.container_module} {
  module @kernels {
    gpu.func @kernel_1(%arg1 : !ptr.ptr) kernel {
      gpu.return
    }
  }

  func.func @launch_func_missing_kernel_attr(%sz : index, %arg : !ptr.ptr) {
    // expected-error@+1 {{kernel module 'kernels' is undefined}}
    gpu.launch_func @kernels::@kernel_1 blocks in (%sz, %sz, %sz) threads in (%sz, %sz, %sz) args(%arg : !ptr.ptr)
    return
  }
}
...
module attributes {gpu.container_module} {
  ...
  func.func @launch_func_kernel_operand_size(%sz : index, %arg : !ptr.ptr) {
    // expected-error@+1 {{got 2 kernel operands but expected 1}}
    gpu.launch_func @kernels::@kernel_1 blocks in (%sz, %sz, %sz) threads in (%sz, %sz, %sz) args(%arg : !ptr.ptr, %arg : !ptr.ptr)
    return
  }
}
