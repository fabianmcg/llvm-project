...
spirv.func @access_chain() "None" {
  ...
  // CHECK: llvm.getelementptr %{{.*}}[%[[ZERO]], 1, %[[ONE]]] : (!ptr.ptr, i32, i32) -> !ptr.ptr, !llvm.struct<packed (f32, array<4 x f32>)>
...
spirv.func @access_chain_array(%arg0 : i32) "None" {
  ...
  // CHECK: %[[ZERO:.*]] = llvm.mlir.constant(0 : i32) : i32
  // CHECK: llvm.getelementptr %{{.*}}[%[[ZERO]], %{{.*}}] : (!ptr.ptr, i32, i32) -> !ptr.ptr, !llvm.array<4 x array<4 x f32>>
...
spirv.module Logical GLSL450 {
  ...
  // CHECK-LABEL: @func
  //       CHECK:   llvm.mlir.addressof @struct : !ptr.ptr
...
spirv.module Logical GLSL450 {
  ...
  // CHECK-LABEL: @foo
  //       CHECK:   llvm.mlir.addressof @bar_descriptor_set0_binding0 : !ptr.ptr
...
spirv.module @name Logical GLSL450 {
  ...
  // CHECK-LABEL: @foo
  //       CHECK:   llvm.mlir.addressof @name_bar_descriptor_set0_binding0 : !ptr.ptr
  spirv.GlobalVariable @bar bind(0, 0) : !spirv.ptr<i32, StorageBuffer>
  spirv.func @foo() "None" {
    %0 = spirv.mlir.addressof @bar : !spirv.ptr<i32, StorageBuffer>
    spirv.Return
...
spirv.func @load_with_alignment() "None" {
  %0 = spirv.Variable : !spirv.ptr<f32, Function>
  // CHECK: llvm.load %{{.*}} {alignment = 4 : i64} : !ptr.ptr -> f32
...
spirv.func @load_volatile() "None" {
  %0 = spirv.Variable : !spirv.ptr<f32, Function>
  // CHECK: llvm.load volatile %{{.*}} : !ptr.ptr -> f32
...
spirv.func @store_composite(%arg0 : !spirv.struct<(f64)>) "None" {
  ...
  // CHECK: llvm.store %{{.*}}, %{{.*}} : !llvm.struct<packed (f64)>, !ptr.ptr
...
spirv.func @store_with_alignment(%arg0 : f32) "None" {
  %0 = spirv.Variable : !spirv.ptr<f32, Function>
  // CHECK: llvm.store %{{.*}}, %{{.*}} {alignment = 4 : i64} : f32, !ptr.ptr
...
spirv.func @store_volatile(%arg0 : f32) "None" {
  %0 = spirv.Variable : !spirv.ptr<f32, Function>
  // CHECK: llvm.store volatile %{{.*}}, %{{.*}} : f32, !ptr.ptr
...
spirv.func @variable_scalar() "None" {
  ...
  // CHECK: llvm.alloca %[[SIZE1]] x f32 : (i32) -> !ptr.ptr
  %0 = spirv.Variable : !spirv.ptr<f32, Function>
  // CHECK: %[[SIZE2:.*]] = llvm.mlir.constant(1 : i32) : i32
  // CHECK: llvm.alloca %[[SIZE2]] x i8 : (i32) -> !ptr.ptr
...
spirv.func @variable_scalar_with_initialization() "None" {
  ...
  // CHECK: %[[ALLOCATED:.*]] = llvm.alloca %[[SIZE]] x i64 : (i32) -> !ptr.ptr
  // CHECK: llvm.store %[[VALUE]], %[[ALLOCATED]] : i64, !ptr.ptr
...
spirv.func @variable_vector() "None" {
  // CHECK: %[[SIZE:.*]] = llvm.mlir.constant(1 : i32) : i32
  // CHECK: llvm.alloca  %[[SIZE]] x vector<3xf32> : (i32) -> !ptr.ptr
...
spirv.func @variable_vector_with_initialization() "None" {
  ...
  // CHECK: %[[SIZE:.*]] = llvm.mlir.constant(1 : i32) : i32
  // CHECK: %[[ALLOCATED:.*]] = llvm.alloca %[[SIZE]] x vector<3xi1> : (i32) -> !ptr.ptr
  // CHECK: llvm.store %[[VALUE]], %[[ALLOCATED]] : vector<3xi1>, !ptr.ptr
...
spirv.func @variable_array() "None" {
  // CHECK: %[[SIZE:.*]] = llvm.mlir.constant(1 : i32) : i32
  // CHECK: llvm.alloca %[[SIZE]] x !llvm.array<10 x i32> : (i32) -> !ptr.ptr
