...
// CHECK: %[[gep:.*]] = llvm.getelementptr %[[null]][%[[one]]] : (!ptr.ptr, i64) -> !ptr.ptr, f32
// CHECK: %[[size_bytes:.*]] = llvm.ptrtoint %[[gep]] : !ptr.ptr to i64
// CHECK: %[[ptr:.*]] = llvm.call @malloc(%[[size_bytes]]) : (i64) -> !ptr.ptr
...
// CHECK: llvm.call @free(%[[ptr]]) : (!ptr.ptr) -> ()
...
// CHECK: %[[null:.*]] = llvm.mlir.zero : !ptr.ptr
// CHECK: %[[gep:.*]] = llvm.getelementptr %[[null]][%[[sz1]]] : (!ptr.ptr, i64) -> !ptr.ptr, f32
// CHECK: %[[size_bytes:.*]] = llvm.ptrtoint %[[gep]] : !ptr.ptr to i64
// CHECK: %[[alignment:.*]] = llvm.mlir.constant(8 : index) : i64
// CHECK: %[[allocsize:.*]] = llvm.add %[[size_bytes]], %[[alignment]] : i64
// CHECK: %[[ptr:.*]] = llvm.call @malloc(%[[allocsize]]) : (i64) -> !ptr.ptr
// CHECK: %[[allocatedAsInt:.*]] = llvm.ptrtoint %[[ptr]] : !ptr.ptr to i64
...
// CHECK: %[[alignedBitCast:.*]] = llvm.inttoptr %[[aligned]] : i64 to !ptr.ptr
...
// CHECK: %[[null:.*]] = llvm.mlir.zero : !ptr.ptr
// CHECK: %[[gep:.*]] = llvm.getelementptr %[[null]][%[[num_elems]]] : (!ptr.ptr, i64) -> !ptr.ptr, f32
// CHECK: %[[size_bytes:.*]] = llvm.ptrtoint %[[gep]] : !ptr.ptr to i64
// CHECK: llvm.call @malloc(%[[size_bytes]]) : (i64) -> !ptr.ptr
...
// CHECK: %[[num_elems:.*]] = llvm.mlir.constant(576 : index) : i64
// CHECK: %[[allocated:.*]] = llvm.alloca %[[num_elems]] x f32 : (i64) -> !ptr.ptr
...
// CHECK: %{{.*}} = llvm.load %[[ptr]] : !ptr.ptr -> f32
...
// CHECK:  %[[addr:.*]] = llvm.getelementptr %[[ptr]][%[[off1]]] : (!ptr.ptr, i64) -> !ptr.ptr, f32
// CHECK:  llvm.load %[[addr]] : !ptr.ptr -> f32
...
// CHECK: llvm.store %{{.*}}, %[[ptr]] : f32, !ptr.ptr
...
// CHECK: %[[off1:.*]] = llvm.add %[[offI]], %[[JJ]] : i64
// CHECK: %[[addr:.*]] = llvm.getelementptr %[[ptr]][%[[off1]]] : (!ptr.ptr, i64) -> !ptr.ptr, f32
// CHECK: llvm.store %{{.*}}, %[[addr]] : f32, !ptr.ptr
...
// CHECK: %[[UB_IDX:.*]] = llvm.getelementptr %{{.*}}[0, %[[C_MINUS_7_I64]]] : (!ptr.ptr, i64) -> !ptr.ptr
// CHECK: %[[UB_DIM_I64:.*]] = llvm.load %[[UB_IDX]] : !ptr.ptr
...
module attributes { dlti.dl_spec = #dlti.dl_spec<#dlti.dl_entry<index, 32>> } {
  func.func @address() {
    ...
    // CHECK: llvm.ptrtoint %{{.*}} : !ptr.ptr to i32
    // CHECK: llvm.ptrtoint %{{.*}} : !ptr.ptr to i32
    // CHECK: llvm.add %{{.*}} : i32
    // CHECK: llvm.call @malloc(%{{.*}}) : (i32) -> !ptr.ptr
    // CHECK: llvm.ptrtoint %{{.*}} : !ptr.ptr to i32
    // CHECK: llvm.sub {{.*}} : i32
    // CHECK: llvm.add {{.*}} : i32
    // CHECK: llvm.urem {{.*}} : i32
    // CHECK: llvm.sub {{.*}} : i32
    // CHECK: llvm.inttoptr %{{.*}} : i32 to !ptr.ptr
    return
  }
}
...
func.func @memref.reshape.dynamic.dim(%arg: memref<?x?x?xf32>, %shape: memref<4xi64>) -> memref<?x?x12x32xf32> {
  ...
  // CHECK: %[[shape_gep0:.*]] = llvm.getelementptr %[[shape_ptr0]][%[[one1]]] : (!ptr.ptr, i64) -> !ptr.ptr, i64
  // CHECK: %[[shape_load0:.*]] = llvm.load %[[shape_gep0]] : !ptr.ptr -> i64
  ...
  // CHECK: %[[shape_gep1:.*]] = llvm.getelementptr %[[shape_ptr1]][%[[zero1]]] : (!ptr.ptr, i64) -> !ptr.ptr, i64
  // CHECK: %[[shape_load1:.*]] = llvm.load %[[shape_gep1]] : !ptr.ptr -> i64
...
func.func @memref.reshape_index(%arg0: memref<?x?xi32>, %shape: memref<1xindex>) ->  memref<?xi32> {
  ...
  // CHECK: %[[shape_ptr0:.*]] = llvm.extractvalue %[[shape_cast:.*]][1] : !llvm.struct<(ptr, ptr, i64, array<1 x i64>, array<1 x i64>)>
  // CHECK: %[[shape_gep0:.*]] = llvm.getelementptr %[[shape_ptr0:.*]][%[[zero1:.*]]] : (!ptr.ptr, i64) -> !ptr.ptr, i64
  // CHECK: %[[shape_load0:.*]] = llvm.load %[[shape_gep0:.*]] : !ptr.ptr -> i64
...
// CHECK: [[CAST_ALLOC:%.*]] = llvm.addrspacecast [[ALLOC]] : !ptr.ptr to !ptr.ptr<1>
// CHECK: [[CAST_ALIGN:%.*]] = llvm.addrspacecast [[ALIGN]] : !ptr.ptr to !ptr.ptr<1>
