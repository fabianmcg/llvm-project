...
static cl::opt<bool> formatErrorIsFatal(
...
static cl::opt<std::string>
    typeDialect("typedefs-dialect",
                cl::desc("Generate types for this dialect"),
...
static cl::opt<std::string>
    selectedDialect("dialect", cl::desc("The dialect to gen for"),
...
static bool withDialectRecords(
  ...
  std::optional<Dialect> dialect = findDialectToGenerate(
      dialects,
...
static GenRegistration
    genDialectDocRegister(
        "gen-dialect-doc", "Generate dialect documentation",
        [](const RecordKeeper &records, raw_ostream &os) {
          return withDialectRecords(records, [&](const DialectRecords &r) {
            return emitDialectDoc(r, stripPrefix, allowHugoSpecificFeatures,
                                  os);
          });
        });
...
static cl::opt<std::string, /*ExternalStorage=*/true>
    clDialectName("bind-dialect",
                  cl::desc("The dialect to run the generator for"),
                  cl::location(dialectNameStorage),
                  cl::cat(clOpPythonBindingCat));

static cl::opt<std::string> clDialectExtensionName(
    "dialect-extension", cl::desc("The prefix of the dialect extension"),
    cl::init(""), cl::cat(clOpPythonBindingCat));

static GenRegistration
    genPythonEnumBindings(
...
static GenRegistration
    genPassDoc("gen-pass-doc", "Generate pass documentation",
               [](const RecordKeeper &records, raw_ostream &os) {
