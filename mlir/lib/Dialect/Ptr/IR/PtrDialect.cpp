...
LogicalResult AtomicRMWOp::verify() {
  ...
  if (failed(getPtr().getType().getMemorySpace().isValidAtomicOp(
          getBinOp(), valueType, getOrdering(), getAlignmentAttr(),
          [&]() -> InFlightDiagnostic { return this->emitError(); })))
    return failure();
...
LogicalResult AtomicCmpXchgOp::verify() {
  Type valueType = getVal().getType();
  if (failed(getPtr().getType().getMemorySpace().isValidAtomicXchg(
...
LogicalResult LoadOp::verify() {
  ...
  if (failed(getAddr().getType().getMemorySpace().isValidLoad(
...
LogicalResult StoreOp::verify() {
  ...
  if (failed(getAddr().getType().getMemorySpace().isValidStore(
          valueType, getOrdering(), getAlignmentAttr(),
          [&]() -> InFlightDiagnostic { return this->emitError(); })))
    return failure();
  return verifyAtomicMemOp(*this, valueType,
...
Value AddrSpaceCastOp::getViewSource() { return getArg(); }

LogicalResult AddrSpaceCastOp::verify() {
  PtrType ptrTy = getUnderlyingPtrType(getArg().getType());
  return ptrTy.getMemorySpace().isValidAddrSpaceCast(
...
LogicalResult IntToPtrOp::verify() {
  PtrType ptrTy = getUnderlyingPtrType(getRes().getType());
  return ptrTy.getMemorySpace().isValidPtrIntCast(
...
void ConstantOp::build(OpBuilder &odsBuilder, OperationState &odsState,
                       int64_t value, MemorySpaceAttrInterface addressSpace) {
