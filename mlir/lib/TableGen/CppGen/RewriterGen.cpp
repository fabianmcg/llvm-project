...
private:
  ...
  // either order.
  void emitEitherOperandMatch(tblgen::DagNode tree, tblgen::DagNode eitherArgTree,
  ...
  // result value name.
  std::string handleOpCreation(tblgen::DagNode tree, int resultIndex, int depth);
...
void PatternEmitter::emit(StringRef rewriteName) {
  ...
  llvm::interleaveComma(sortedResultOps, os,
...
std::string PatternEmitter::handleOpCreation(tblgen::DagNode tree,
  ...
  auto checkIfMatchedVariadic = [&](int i) {
    ...
    const auto *operand = llvm::dyn_cast_if_present<tblgen::NamedTypeConstraint *>(
        resultOp.getArg(i));
...
void StaticMatcherHelper::addPattern(const Record *record) {
  ...
  // order.
  llvm::unique_function<void(tblgen::DagNode)> dfs =
      [&](tblgen::DagNode node) {
        ++refStats[node];
