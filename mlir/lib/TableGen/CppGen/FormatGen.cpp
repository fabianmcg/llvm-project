...
OptionalElement::getThenElements(bool parseable) const {
  return llvm::ArrayRef(thenElements).drop_front(parseable ? thenParseStart
                                                           : 0);
}

ArrayRef<FormatElement *>
OptionalElement::getElseElements(bool parseable) const {
  return llvm::ArrayRef(elseElements).drop_front(parseable ? elseParseStart
                                                           : 0);
}
...
FormatParser::parseCustomDirective(SMLoc loc, Context ctx) {
...
FailureOr<FormatElement *>
FormatParser::parseRefDirective(SMLoc loc, Context context) {
...
FailureOr<FormatElement *>
FormatParser::parseQualifiedDirective(SMLoc loc, Context ctx) {
  if (failed(parseToken(FormatToken::l_paren,
                        "expected '(' before argument list")))
    return failure();
