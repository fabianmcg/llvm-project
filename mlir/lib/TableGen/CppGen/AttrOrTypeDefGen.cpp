...
void AttrOrTypeDefEmitter::emitInvariantsVerifier(bool hasImpl, bool hasCustomVerifier) {
...
AttrOrTypeDefEmitter::getBuilderParams(std::initializer_list<MethodParameter> prefix) const {
...
void AttrOrTypeDefEmitter::emitCheckedCustomBuilder(const AttrOrTypeBuilder &builder) {
  // Don't emit a body if there isn't one.
  auto props = builder.getBody() ? Method::Static : Method::StaticDeclaration;
  StringRef returnType = def.getCppClassName();
  if (std::optional<StringRef> builderReturnType = builder.getReturnType())
...
void AttrOrTypeDefEmitter::genTraitMethodUsingDecl(const InterfaceTrait &trait,
...
AttrDefGenerator::AttrDefGenerator(const RecordKeeper &records,
                                   raw_ostream &os, bool fatalOnError)
    : DefGenerator(records.getAllDerivedDefinitionsIfDefined("AttrDef"), os,
                   "Attr", "Attribute", /*isAttrGenerator=*/true, fatalOnError) {
}

TypeDefGenerator::TypeDefGenerator(const RecordKeeper &records,
                                   raw_ostream &os, bool fatalOnError)
    : DefGenerator(records.getAllDerivedDefinitionsIfDefined("TypeDef"), os,
                   "Type", "Type", /*isAttrGenerator=*/false, fatalOnError) {
}
