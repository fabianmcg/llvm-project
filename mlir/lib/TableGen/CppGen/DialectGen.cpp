...
mlir::tblgen::findDialectToGenerate(llvm::ArrayRef<Dialect> dialects,
  ...
  const auto *dialectIt =
      llvm::find_if(dialects, [&](const Dialect &dialect) {
...
bool mlir::tblgen::emitDialectDecls(const RecordKeeper &records,
  ...
  std::optional<Dialect> dialect = findDialectToGenerate(dialects, selectedDialect);
  if (!dialect)
    return true;
...
void mlir::tblgen::emitDialectDef(Dialect &dialect,
