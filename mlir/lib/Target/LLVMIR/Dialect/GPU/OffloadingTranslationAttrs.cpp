...
OffloadManager::getOffloadingImage(gpu::ObjectAttr obj) {
  auto targetAttr = cast<gpu::TargetAttrInterface>(obj.getTarget());
...
OffloadManager::bundleGPU(ArrayRef<Attribute> objects) {
  ...
  SmallVector<std::unique_ptr<llvm::MemoryBuffer>> buffers;
  for (Attribute attr : objects) {
    auto object = cast<gpu::ObjectAttr>(attr);
...
LogicalResult OffloadManager::emitOffloadingEntry(gpu::LaunchFuncOp launchFunc,
                                                  llvm::Constant *stubPtr) {
  ...
  std::pair<llvm::Constant *, llvm::GlobalVariable *> entry =
      llvm::offloading::getOffloadingEntryInitializer(
          module, stubPtr, launchFunc.getKernelName().getValue(), 0, 0, 0);
...
LogicalResult OffloadManager::wrapImages(llvm::Module &module,
  ...
  StringRef identifier = binaryOp.getName();
  OffloadEntryArray entryArray = emitEmptyEntryArray(getSymbolSuffix());
  switch (offloadKind) {
  case gpu::OffloadKind::OpenMP:
    if (auto error = llvm::offloading::OffloadWrapper("." + identifier)
                         .wrapOpenMPBinaries(module, imgs, entryArray))
      return binaryOp.emitError("failed wrapping the OpenMP binaries");
    return success();
  case gpu::OffloadKind::CUDA:
    if (auto error = llvm::offloading::OffloadWrapper("." + identifier, false)
                         .wrapCudaBinary(module, imgs.front(), entryArray))
      return binaryOp.emitError("failed wrapping the CUDA binaries");
    return success();
  case gpu::OffloadKind::HIP:
    if (auto error = llvm::offloading::OffloadWrapper("." + identifier, false)
                         .wrapHIPBinary(module, imgs.front(), entryArray))
...
OffloadManager::launchKernel(gpu::LaunchFuncOp launchFunc,
  ...
  std::string stubName = entryId + "_stub";
  llvm::Value *stubPtr = nullptr;
  if (!(stubPtr = hostModule->getFunction(stubName))) {

    llvm::FunctionCallee fnCallee = hostModule->getOrInsertFunction(
        stubName, llvm::FunctionType::get(builder.getVoidTy(), false));

    auto fn = mlir::dyn_cast<llvm::Function>(fnCallee.getCallee());
    {
      llvm::IRBuilder<> localBuilder(
          llvm::BasicBlock::Create(builder.getContext(), "entry", fn));
      localBuilder.CreateRetVoid();
    }
    fn->setDSOLocal(true);
    if (failed(emitOffloadingEntry(launchFunc, fn)))
      return failure();
    stubPtr = fnCallee.getCallee();
  }
  return llvm::LaunchKernel(*moduleTranslation.getLLVMModule(), builder,
                            moduleTranslation)
      .createKernelLaunch(launchFunc, nullptr, stubPtr);
}
