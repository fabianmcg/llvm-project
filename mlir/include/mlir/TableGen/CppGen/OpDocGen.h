...
#ifndef MLIR_TABLEGEN_CPPGEN_OPDOCGEN_H
#define MLIR_TABLEGEN_CPPGEN_OPDOCGEN_H
...
/// generation. \p opDefs are the op definitions to include (e.g. filtered by
/// the caller). \p dialect is the dialect to collect records for.
/// \p keepOpSourceOrder disables alphabetical sorting of ops.
...
/// Emit documentation for a single operation. \p stripPrefix is stripped from
/// the fully qualified class name. \p allowHugoSpecificFeatures enables
...
/// Emit operation documentation for all ops in \p records.
bool emitOpDoc(const DialectRecords &records, llvm::StringRef stripPrefix,
               bool allowHugoSpecificFeatures, llvm::raw_ostream &os);

/// Emit attribute definition documentation for all attrDefs in \p records.
bool emitAttrDefDoc(const DialectRecords &records, llvm::raw_ostream &os);

/// Emit type definition documentation for all typeDefs in \p records.
bool emitTypeDefDoc(const DialectRecords &records, llvm::raw_ostream &os);

/// Emit enum documentation for all enums in \p records.
...
#endif // MLIR_TABLEGEN_CPPGEN_OPDOCGEN_H
